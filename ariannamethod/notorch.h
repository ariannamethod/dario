...
// C[m,n] = A[m,k] @ B[n,k]^T  (B is stored row-major with n rows of k cols)
// Uses BLAS when available, falls back to naive loops.
void nt_blas_mmT(float *C, const float *A, const float *BT, int m, int k, int n);

// C[m,n] = A[m,k] @ B[k,n]  (standard matmul, B not transposed)
