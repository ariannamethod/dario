...
> Note on this edition. This second edition is not a revision of the first paper but a deeper account of the same system, written after we rebuilt the part of it the first edition got wrong. The first edition measured Dario and reported what it saw; one of those readings — that a single force, destiny, dominated the equation — was an artifact of how we measured, not a fact of the architecture. We rebuilt the force mechanisms until the measurement was honest, re-ran the entire experimental frame on the same hardware, and report the corrected readings here with their statistics. The first edition stands as history. This edition is where the system actually is.
